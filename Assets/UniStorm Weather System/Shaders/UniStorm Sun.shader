...
Properties {
	...
	_OpaqueY("Opaque Y", Float) = 20
	_TransparentY("Transparent Y", Float) = 0
	_FadeControl ("Fade Control", Range(0, 10)) = 5
	_PlayerY ("Player Y Position", Float) = 0.0
}
	...
	SubShader {
		Pass {
			...
			v2f vert (appdata_t v)
			{
				...
				float fadeRange = _OpaqueY - _TransparentY;
                float heightFactor = (_OpaqueY - (worldV.y- _PlayerY)) / fadeRange;
                float fadeFactor = saturate(1.0 - (_FadeControl / 10.0) * heightFactor);
